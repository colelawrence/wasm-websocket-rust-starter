...
export class Code {
  indentation = "  " + d`$$`;
  static group(lines?: (Code | string)[]): Code {
    return new Code(lines, true);
  }
  static indented(lines?: (Code | string)[]): Code {
    return new Code(lines, false);
  }
  static fmt(template: TemplateStringsArray, ...args: Args): string {
    return raw(template, ...args);
  }
  /** @deprecated in favor of {@link Code.indented} and {@link Code.group} */
  constructor(
  ...
  static docString(docs: gen.Attrs, extraLine?: string, includeLocationID?: [string, gen.LocationID]): string[] {
    ...
    if (found) {
      const multiLinePrefix = (Code.docStringSettings.multi_line.prefix + Code.docStringSettings.multi_line.line_pre).replace(/^\n/g, "");
      return [
        found.includes("\n")
          ? multiLinePrefix +
            found.trim().replace(/\n([^\n]*)/g, (_, capture) => {
  ...
  dbg(arr: TemplateStringsArray, ...args: Args) {
    if (Code.debug) {
      this.lines.push(raw(arr, ...args));
    }
  }
